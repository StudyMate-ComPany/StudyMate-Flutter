...
class FlutterNaverLoginPlugin : FlutterPlugin, MethodCallHandler, ActivityAware {
    ...
    private val MIN_LOGIN_INTERVAL = 1000L // 최소 1초 간격
    ...
    // in the same class.
    // Deprecated registerWith method commented out for compatibility
    // companion object {
    //     @JvmStatic
    ...
    private fun initSdk(
        ...
        try {
            ...
            println("- packageName: ${applicationContext.packageName}")

            // URL 스킴을 패키지명으로 명시적 설정
            NaverIdLoginSDK.initialize(applicationContext, clientId, clientSecret, clientName)
            
            // 개발자 로그 확인
            println("- OAuth redirectURI: ${applicationContext.packageName}://authorize")
            result.success(true)
    ...
    private fun login(result: Result) {
        ...
        if (isLoginInProgress) {
            println("[NaverLogin] Login already in progress, ignoring duplicate request")
            result.error("LOGIN_IN_PROGRESS", "Login is already in progress", null)
            return
        }
        
        // 너무 빠른 연속 호출 방지
        if (currentTime - lastLoginAttemptTime < MIN_LOGIN_INTERVAL) {
            println("[NaverLogin] Too rapid login attempts, ignoring")
            result.error("TOO_RAPID", "Too rapid login attempts", null)
            return
        }
        
        // 이미 로그인되어 있으면 현재 계정 정보 반환
        if (NaverIdLoginSDK.getAccessToken() != null) {
            println("[NaverLogin] Already logged in, returning current account")
            mainScope.launch {
                getCurrentAccount(result)
            }
            return
        ...
        val mOAuthLoginHandler = object : OAuthLoginCallback {
            override fun onSuccess() {
                println("[NaverLogin] OAuth Success")
            ...
            override fun onFailure(httpStatus: Int, message: String) {
                println("[NaverLogin] OAuth Failure: $message")
                isLoginInProgress = false
                pendingResult = null
                val errorCode = NaverIdLoginSDK.getLastErrorCode().code
                val errorDesc = NaverIdLoginSDK.getLastErrorDescription()
            ...
            override fun onError(errorCode: Int, message: String) {
                println("[NaverLogin] OAuth Error: $message")
                isLoginInProgress = false
                pendingResult = null
        ...
        try {
            ...
            println("[NaverLogin] Exception during authenticate: ${e.message}")
            isLoginInProgress = false
            pendingResult = null
